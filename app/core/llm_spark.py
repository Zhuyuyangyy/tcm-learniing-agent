...
# ============================================================
# Dify 自动检测（优先读 config.yaml，env 可覆盖）
# ============================================================

_DIFY_LLM_CACHE: Any = None  # False=检测过但未配置, None=未检测, obj=已就绪
...
def _get_dify_llm_if_configured() -> Optional[Any]:
    """检测到 Dify 已配置时返回 DifyWorkflowLLM，否则 None"""
    global _DIFY_LLM_CACHE
    if _DIFY_LLM_CACHE is False:
        return None  # 已检测过但未配置
    ...
    try:
        ...
        logger.info(f"✅ Dify LLM 已就绪（api_url={api_url}），讯飞星火将切换为 Dify")
        return _DIFY_LLM_CACHE
    except Exception as e:
        logger.warning(f"⚠️ Dify LLM 初始化失败，降级到讯飞星火: {e}")
        _DIFY_LLM_CACHE = False
        return None
...
def get_crewai_llm(temperature: float = 0.7) -> Any:
    ...
    切换逻辑（优先级从高到低）：
    1. env 中 DIFY_API_URL + DIFY_API_KEY
    2. config.yaml 中 llm.dify.enabled=true + api_url/api_key
    3. config.yaml 中 llm.provider == "dify" 且已配置
    4. 否则 → 讯飞星火 SparkChatModel
    """
    dify_llm = _get_dify_llm_if_configured()
    if dify_llm is not None:
        logger.info(f"✅ CrewAI LLM 使用 Dify | temp={temperature}")
        return dify_llm

    # 检查 config.yaml 中 provider 是否强制为 dify
    cfg = _load_config()
    llm_cfg = cfg.get("llm", {})
    if llm_cfg.get("provider") == "dify":
        dify_llm = _get_dify_llm_if_configured()
        if dify_llm is not None:
            logger.info(f"✅ CrewAI LLM 使用 Dify（config.yaml指定provider=dify）| temp={temperature}")
            return dify_llm
        logger.warning("⚠️ config.yaml 指定 provider=dify 但 Dify 未配置，降级到讯飞星火")
